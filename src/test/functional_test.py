...
class UploadFileTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        ret = test_sdk.FindLeader(self.host, self.sentinel_ports)
        self.assertNotEqual(ret, None)
        self.get_oplog_port = 7070 + (ret[1]+1)*10000
    ...
    def get_new_request(self):
        ...
        uf_request.account = "upload_file_test"
        UploadFileTest.seqnum = UploadFileTest.seqnum+1
        uf_request.file_name = "test%d" % (UploadFileTest.seqnum, )
        uf_request.merkle_sha1 = "sha1"
        uf_request.file_size = 1024
        return uf_request

    def get_prev_key(self, last = 1):
        return "upload_file_test" + "test%d" % (UploadFileTest.seqnum-last+1, )
...
class GetOplogTest(unittest.TestCase):
    ...
    def get_new_request(self):
        uf_request = upload_file_pb2.UploadFileRequest()
        uf_request.account = "get_oplog_test"
    ...
    def get_prev_key(self, last = 1):
        return "get_oplog_test" + "test%d" % (GetOplogTest.seqnum-last+1, )
